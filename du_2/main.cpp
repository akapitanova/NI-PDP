...
public:
    ...
    Graph(int n, int k, int a, int level_coef, std::vector<std::vector<int>> edges_w){
        this->n = n;
        this->k = k;
        this->a = a;

        this->max_level = k * level_coef;
    ...
    void make_min_edge_cut(){
        ...
        #pragma omp parallel shared (best_price, best_sol, calls, X_vertices)
        {
        #pragma omp single
...
private:
    ...
    void solve_seq(int vertex, std::vector<int>& X_vertices, int ones, int price){
        #pragma omp atomic
        calls += 1;
...
// BB-DFS pro hledani minimalniho hranoveho rezu
    void solve(int vertex, std::vector<int>& X_vertices, int ones, int price){
        ...
        if (vertex < max_level){
            ...
            solve_seq(vertex + 1, X_vertices_left, ones, price_left);
        }
        ...
        if (vertex < max_level){
            ...
            solve_seq(vertex + 1, X_vertices_right, ones + 1, price_right);
        }
...
Graph* load_data(std::string& dir_path, std::string& file_name, int a, int level_coef){
    if (a < 5)
        return nullptr;
    ...
    Graph * graph = new Graph(n, k, a, level_coef, edges_w);
    return graph;
}
...
int main(int argc, char * argv[]) {
    ...
    int level_coef = std::stoi(argv[3]);

    Graph* graph = load_data(dir_path, file_name, a, level_coef);
    if (!graph){
        return 1;
    }
